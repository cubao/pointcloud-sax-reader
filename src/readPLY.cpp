...
IGL_INLINE bool _tinyply_buffer_to_matrix(tinyply::PlyData& D, Eigen::PlainObjectBase<Derived>& M,
                                          size_t rows, size_t cols) {
  Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> > _map(
      reinterpret_cast<T*>(D.buffer.get()), rows, cols);
...
IGL_INLINE bool tinyply_buffer_to_matrix(tinyply::PlyData& D, Eigen::PlainObjectBase<Derived>& M,
                                         size_t rows, size_t cols) {
  switch (D.t) {
    case tinyply::Type::INT8: return _tinyply_buffer_to_matrix<int8_t, Derived>(D, M, rows, cols);
    case tinyply::Type::UINT8: return _tinyply_buffer_to_matrix<uint8_t, Derived>(D, M, rows, cols);
    case tinyply::Type::INT16: return _tinyply_buffer_to_matrix<int16_t, Derived>(D, M, rows, cols);
    case tinyply::Type::UINT16:
      return _tinyply_buffer_to_matrix<uint16_t, Derived>(D, M, rows, cols);
    case tinyply::Type::INT32: return _tinyply_buffer_to_matrix<int32_t, Derived>(D, M, rows, cols);
    case tinyply::Type::UINT32:
      return _tinyply_buffer_to_matrix<uint32_t, Derived>(D, M, rows, cols);
    case tinyply::Type::FLOAT32: return _tinyply_buffer_to_matrix<float, Derived>(D, M, rows, cols);
...
template <typename T, typename Derived>
IGL_INLINE bool _tinyply_tristrips_to_trifaces(tinyply::PlyData& D,
                                               Eigen::PlainObjectBase<Derived>& M, size_t el,
                                               size_t el_len) {
  Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> > _map(
      reinterpret_cast<T*>(D.buffer.get()), el, el_len);

  // to make it more interesting, triangles in triangle strip can be separated by negative index
  // elements
  ...
  for (size_t i = 0; i < el; i++)
    for (size_t j = 0; j < (el_len - 2); j++) {
      if (_map(i, j) >= 0 && _map(i, j + 1) >= 0 && _map(i, j + 2) >= 0) triangles++;
    }
...
template <typename Derived>
IGL_INLINE bool tinyply_tristrips_to_faces(tinyply::PlyData& D, Eigen::PlainObjectBase<Derived>& M,
                                           size_t el, size_t el_len) {
  switch (D.t) {
...
template <typename DerivedV, typename DerivedF, typename DerivedE, typename DerivedN,
          typename DerivedUV, typename DerivedVD, typename DerivedFD, typename DerivedED>
IGL_INLINE bool readPLY(FILE* fp, Eigen::PlainObjectBase<DerivedV>& V,
                        Eigen::PlainObjectBase<DerivedF>& F, Eigen::PlainObjectBase<DerivedE>& E,
                        Eigen::PlainObjectBase<DerivedN>& N, Eigen::PlainObjectBase<DerivedUV>& UV,

                        Eigen::PlainObjectBase<DerivedVD>& VD, std::vector<std::string>& Vheader,
                        Eigen::PlainObjectBase<DerivedFD>& FD, std::vector<std::string>& Fheader,
                        Eigen::PlainObjectBase<DerivedED>& ED, std::vector<std::string>& Eheader,
...
template <typename DerivedV, typename DerivedF, typename DerivedE, typename DerivedN,
          typename DerivedUV, typename DerivedVD, typename DerivedFD, typename DerivedED>
IGL_INLINE bool readPLY(std::istream& ply_stream, Eigen::PlainObjectBase<DerivedV>& V,
                        Eigen::PlainObjectBase<DerivedF>& F, Eigen::PlainObjectBase<DerivedE>& E,
                        Eigen::PlainObjectBase<DerivedN>& N, Eigen::PlainObjectBase<DerivedUV>& UV,
  ...
  std::set<std::string> vertex_std{"x", "y", "z",         "nx",        "ny", "nz",
                                   "u", "v", "texture_u", "texture_v", "s",  "t"};
  ...
  for (auto c : file.get_comments()) comments.push_back(c);
  ...
  if (!faces ||
      !tinyply_buffer_to_matrix(
          *faces, F, faces->count,
          faces->count == 0 ? 0
                            : faces->buffer.size_bytes() /
                                  (tinyply::PropertyTable[faces->t].stride * faces->count))) {
    if (tristrips) {  // need to convert to faces
      ...
      size_t el_count = tristrips->buffer.size_bytes() /
                        (tinyply::PropertyTable[tristrips->t].stride * tristrips->count);
...
template <typename DerivedV, typename DerivedF, typename DerivedE, typename DerivedN,
          typename DerivedUV, typename DerivedVD, typename DerivedFD, typename DerivedED>
IGL_INLINE bool readPLY(const std::string& ply_file, Eigen::PlainObjectBase<DerivedV>& V,
                        Eigen::PlainObjectBase<DerivedF>& F, Eigen::PlainObjectBase<DerivedE>& E,
                        Eigen::PlainObjectBase<DerivedN>& N, Eigen::PlainObjectBase<DerivedUV>& UV,

                        Eigen::PlainObjectBase<DerivedVD>& VD, std::vector<std::string>& VDheader,

                        Eigen::PlainObjectBase<DerivedFD>& FD, std::vector<std::string>& FDheader,

                        Eigen::PlainObjectBase<DerivedED>& ED, std::vector<std::string>& EDheader,
                        std::vector<std::string>& comments) {
...
template <typename DerivedV, typename DerivedF, typename DerivedE, typename DerivedN,
          typename DerivedUV, typename DerivedD>
IGL_INLINE bool readPLY(const std::string& filename, Eigen::PlainObjectBase<DerivedV>& V,
                        Eigen::PlainObjectBase<DerivedF>& F, Eigen::PlainObjectBase<DerivedE>& E,
                        Eigen::PlainObjectBase<DerivedN>& N, Eigen::PlainObjectBase<DerivedUV>& UV,
                        Eigen::PlainObjectBase<DerivedD>& VD, std::vector<std::string>& Vheader) {
...
IGL_INLINE bool readPLY(const std::string& filename, Eigen::PlainObjectBase<DerivedV>& V,
                        Eigen::PlainObjectBase<DerivedF>& F, Eigen::PlainObjectBase<DerivedN>& N,
                        Eigen::PlainObjectBase<DerivedUV>& UV) {
  Eigen::MatrixXd VD, FD, ED;
  Eigen::MatrixXi E;
  std::vector<std::string> Vheader, Fheader, Eheader;
...
template bool
igl::readPLY<Eigen::Matrix<double, -1, 3, 1, -1, 3>, Eigen::Matrix<int, -1, -1, 0, -1, -1> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 1, -1, 3> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&);
template bool
igl::readPLY<Eigen::Matrix<float, -1, 3, 1, -1, 3>, Eigen::Matrix<int, -1, -1, 0, -1, -1> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<float, -1, 3, 1, -1, 3> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&);
template bool
igl::readPLY<Eigen::Matrix<double, -1, 3, 0, -1, 3>, Eigen::Matrix<int, -1, 3, 0, -1, 3> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 0, -1, 3> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 0, -1, 3> >&);
template bool
igl::readPLY<Eigen::Matrix<double, -1, 3, 1, -1, 3>, Eigen::Matrix<int, -1, 3, 1, -1, 3> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 3, 1, -1, 3> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 1, -1, 3> >&);
template bool
igl::readPLY<Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 0, -1, -1> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&);
template bool
igl::readPLY<Eigen::Matrix<double, -1, -1, 1, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<double, -1, -1, 1, -1, -1> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&);
template bool
igl::readPLY<Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1> >(
    ...
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&);
template bool
igl::readPLY<Eigen::Matrix<float, -1, 3, 0, -1, 3>, Eigen::Matrix<int, -1, 3, 0, -1, 3> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<float, -1, 3, 0, -1, 3> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 0, -1, 3> >&);
template bool
igl::readPLY<Eigen::Matrix<float, -1, 3, 1, -1, 3>, Eigen::Matrix<unsigned int, -1, 3, 1, -1, 3> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<float, -1, 3, 1, -1, 3> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<unsigned int, -1, 3, 1, -1, 3> >&);
template bool
igl::readPLY<Eigen::Matrix<float, -1, 3, 1, -1, 3>, Eigen::Matrix<int, -1, 3, 1, -1, 3> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<float, -1, 3, 1, -1, 3> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, 3, 1, -1, 3> >&);
template bool
igl::readPLY<Eigen::Matrix<float, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1> >(
    FILE*, Eigen::PlainObjectBase<Eigen::Matrix<float, -1, -1, 0, -1, -1> >&,
    Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&);
